{
  aspects.homeManager.niri =
    ...
    in
    {
      # Manual KDL config (niri-flake pins stale v25.08, missing modern actions).
      xdg.configFile."niri/config.kdl".text = ''
        input {
            keyboard { xkb { layout "us"; }; repeat-delay 600; repeat-rate 25; }
            touchpad { tap; natural-scroll; click-method "clickfinger"; }
            warp-mouse-to-focus; focus-follows-mouse; workspace-auto-back-and-forth;
        }
        layout {
            gaps 12; background-color "#1e1e2e";
            focus-ring { width 3; active-color "#cba6f7"; }
            border { off; }
            default-column-width { proportion 0.5; }
            center-focused-column "always";
        }
        cursor { xcursor-theme "catppuccin-mocha-mauve-cursors"; xcursor-size 24; }
        screenshot-path "~/Pictures/Screenshots/%Y-%m-%d %H-%M-%S.png";
        prefer-no-csd;
        environment { QT_QPA_PLATFORM "wayland;xcb"; QT_WAYLAND_DISABLE_WINDOWDECORATION "1"; }
      ...
      # Noctalia shell: bar, launcher, notifications, OSD, lock, wallpaper, night light.
      programs.noctalia = {
        enable = true;
        systemd.enable = true;
        settings = {
