            ...
            systemd.services."restic-backups-${hostName}" = {
              serviceConfig = lib.mkMerge [
                ...
                })
                {
                  ...
                  # resets both to success/0 without re-running anything --
                  # so a freshness check that trusted them could report a
                  # genuinely failed backup as fresh. This ExecStart entry is
                  # deliberately the *last* one (lib.mkAfter appends it after
                  # restic's own backup/unlock/forget/check ExecStart lines,
                  # generated by services.restic.backups above): a oneshot
                  # unit runs its ExecStart entries in order and stops at the
                  # first failure, so reaching -- and only updating the
                  # marker's mtime at -- this line is itself proof that every
                  # prior command already exited 0.
                  StateDirectory = "restic-backups-${hostName}";
                  ExecStart = lib.mkAfter [
                    "${pkgs.coreutils}/bin/touch /var/lib/restic-backups-${hostName}/last-success"
                  ];
                }
              ];
            ...
            systemd.services."btrbk-${hostName}" = {
              serviceConfig = lib.mkMerge [
                ...
                })
                {
                  ...
                  # once btrbk's own run command has exited 0. StateDirectory
                  # is a list merge (lib.mkAfter) alongside the upstream
                  # services.btrbk module's own "btrbk" state dir, adding a
                  # second, dedicated directory rather than replacing it.
                  StateDirectory = lib.mkAfter [ "btrbk-${hostName}" ];
                  ExecStart = lib.mkAfter [
                    "${pkgs.coreutils}/bin/touch /var/lib/btrbk-${hostName}/last-success"
                  ];
                }
              ];
