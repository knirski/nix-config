...
| `source-filter-contract` | Nix-backed checks preserve tracked files while excluding local agent settings, nested worktree metadata, and cache artifacts | `perSystem.nix` | Pure eval |
| `soyo-guest-coverage` | Every systemd service on Soyo is classified — either in the shared guest list (`lib/soyo-guest-units.nix`, isolated by `soyo-guest-isolation`) or in `soyo-guest-coverage.nix`'s `nonGuestUnits` (critical role / operator recovery / infrastructure, each with an inline reason) or matched by a tight `nonGuestPatterns` (`systemd-*`, `getty`); a future aspect that lands an unisolated service fails CI with a targeted remediation hint | `soyo-guest-coverage.nix` | Pure eval |
