...
This repository has one owned dependency surface:

- **Nix inputs**, pinned by `flake.lock` and updated via Renovate's `flake`
  manager (see `renovate.json`) or `nix flake update <input>`. Every input
  either follows `nixpkgs`/`nixpkgs-unstable` or is deliberately pinned for a
  reason recorded at its declaration in `flake.nix` — `opencode-v2` is the
  current example, because its fixed-output `node_modules` hash is computed
  against the bun version in that input's own `flake.lock`.

GitHub's dependency-review action does not understand `flake.lock`, so
reviewing lock diffs remains the manual process described above. Automatic
dependency merging is out of scope: reproducibility is preserved by reviewing
lock changes and validating built systems, not by accepting updates
unattended.
