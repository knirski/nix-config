...
`secrets/rekeyed/soyo/` currently still contains a leftover rekeyed
`github-token.age` from before the secret was scoped to workstation hosts.
Nothing under `secrets/` or `secrets/rekeyed/` is hand-edited or deleted by
this repo's tooling — a stale rekeyed file for a secret soyo no longer
declares is inert (soyo's `config.age.secrets` no longer references it, so it
is never decrypted or placed under `/run/agenix` on soyo) and is cleaned up
later, under separate operator authorization, the next time `agenix rekey`
runs for real.
