...
├── soyo-ntfy-token.age     # Master-encrypted (soyo's ntfy.sh access token)
├── soyo-ntfy-topic.age     # Master-encrypted (soyo's ntfy.sh topic URL)
...
├── github-token.age          # Master-encrypted (GitHub token; workstation-only, see below)
├── zbook-ntfy-token.age      # Master-encrypted (zbook's own ntfy.sh access token)
├── zbook-ntfy-topic.age      # Master-encrypted (zbook's own ntfy.sh topic URL)
└── rekeyed/
    ...
    │   ├── f8f0fa4b0eb42c445dc41ab35065dd1c-soyo-ntfy-token.age
    │   ├── 775ab422d9525ef623929b603207f121-soyo-ntfy-topic.age
    │   ├── ef4130293373fab8ccefb0195d4a82ba-grafana-admin-password.age
    │   └── 97e1e26111fa781025bc9b5049d5d73f-grafana-secret-key.age
    └── zbook/              # Host-specific rekeyed files (zbook's key)
        ├── ...-tailscale-auth-key.age
        ├── ...-zbook-restic-password.age
        ├── ...-github-token.age
        ├── ...-zbook-ntfy-token.age
        └── ...-zbook-ntfy-topic.age
```

`agenix rekey` manages `secrets/rekeyed/` itself: rekeyed copies of secrets a
host no longer declares are removed automatically. This run removed both
hosts' rekeyed copies of the old shared `ntfy-token`/`ntfy-topic` (renamed to
`soyo-ntfy-*` and rekeyed under the new names — every host's ntfy secrets are
now host-specific, see below), so no leftover rekeyed files remain. Nothing
under `secrets/` or `secrets/rekeyed/` is hand-edited or deleted by this
repo's tooling.
...
| `secrets/soyo-ntfy-token.age` | soyo's ntfy.sh access token (soyo-alerts-* topic) | master identity |
| `secrets/soyo-ntfy-topic.age` | soyo's ntfy.sh topic URL (soyo-alerts-* topic) | master identity |
...
### Where `github-token` is declared
...
Instead, `age.secrets.github-token` is declared directly in
`modules/parts/zbook.nix`'s per-host block, alongside `tailscale-auth-key` and
`zbook-restic-password`. Any future host that enables
`aspects.homeManager.development` (e.g. a future dev host) should declare this
secret the same way, in its own assembler's per-host block — not by adding it
back to the shared `users` aspect. soyo's evaluated `config.age.secrets` does
not contain `github-token` at all.

Every host's ntfy alerts go to its own ntfy.sh channel, so the ntfy secrets
are host-specific like `github-token`: `soyo-ntfy-token`/`soyo-ntfy-topic`
(channel `soyo-alerts-*`, also read by Grafana alerting on soyo) are declared
in `modules/parts/soyo.nix`, and `zbook-ntfy-token`/`zbook-ntfy-topic`
(channel `zbook-alerts-*`) in `modules/parts/zbook.nix`. The maintenance
aspect learns the names per host via the required
`lanAppliance.services.maintenance.ntfyTokenSecret`/`ntfyTopicSecret`
options, set in each `hosts/<host>/maintenance.nix`. The shared `users`
aspect therefore carries no ntfy secrets at all — never add a host's channel
back there.
...
echo -n "new-value" \
  | nix run nixpkgs#rage -- -e -i /etc/agenix-rekey/master-identity \
    -o secrets/soyo-ntfy-token.age

# 3. Rekey for all hosts
nix develop '.#' -c agenix rekey

# 4. Commit and deploy
git add secrets/soyo-ntfy-token.age secrets/rekeyed/
git commit -m "chore: update soyo ntfy token"
nixos-rebuild switch --flake .#soyo --target-host krzysiek@soyo --sudo
```
