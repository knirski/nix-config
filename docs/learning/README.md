...
| 31 | SSH key rotation | M4 (shipped) | The master key (`~/.ssh/agenix_master`) and SSH login key (`krzysiek-authorized-key.pub`) are now separate. Rotate each independently: the master key requires re-encrypting all `.age` files and rekeying; the login key only requires updating `krzysiek-authorized-key.pub` and redeploying |
...
| 33 | agenix native SSH recipients | M4 (shipped) | Master `.age` files use raw SSH pubkeys (`rage -e -r "ssh-ed25519 AAAA..."`), stored in `secrets/agenix-master.pub`. No `ssh-to-age` conversion needed — rage handles SSH keys natively. The master key (`agenix_master`) is separate from the SSH login key (`krzysiek-authorized-key.pub`) |
