...
drift). The `Strict KVM behavior tests` job is enforced by the active
`Protect main` ruleset; see
...
For `macbook` and `ubuntu`, the platform-aware branch checks the Nix
installation, current Home Manager profile, SSH configuration, and the native
Darwin/Ubuntu platform marker. It intentionally does not pretend those hosts
have the NixOS systemd, DNS/DHCP, or appliance NIC checks.
...
| **Observability alerting scope** | Grafana routes critical service, backup, Btrfs, Prometheus scrape, Loki availability, and Tempo availability alerts through ntfy; `Reliability Overview` records DNS/DHCP, scrape, last-backup, and target-availability indicators | Remaining improvement: validate alert thresholds during a live drill |
...
- Added ntfy-routed alerts for critical Prometheus scrape failures, Loki
  availability, and Tempo availability alongside the existing service alerts.
- Added the `Reliability Overview` dashboard for DNS/DHCP, scrape, last-backup,
  and target availability indicators. It is intentionally an indicator
  dashboard, not a formal error-budget implementation.
